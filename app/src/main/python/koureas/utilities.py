...
def saveWatermarkedImage(watermarkedImageName, watermarkedImage, dictionary, code, codeSips, flag):
	...
	watermarkedImage.save((os.path.join(subpath, (watermarkedImageName + ".jpg"))), quality = 100)
	mapping_path = "Code_Mapping.txt"
	encryption_path = "encryption_key.txt"
...
def writeGridPlacementsInFile(allGridPositions):
	filename = "config.txt"
	encryption_path = "encryption_key_config.txt"
...
def readGridlacementsFromFile():
	...
	encrypted_file_path = 'config.txt.encrypted'
	with open('encryption_key_config.txt', 'rb') as file:
		key = file.read()
