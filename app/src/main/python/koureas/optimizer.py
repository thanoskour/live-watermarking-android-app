...
def getPSNRAndSSIM(g_cell, watermarkedBlock):
	psnr = PSNR(np.array(g_cell), np.array(watermarkedBlock))
	ssim = SSIM(np.array(g_cell), np.array(watermarkedBlock))
...
def getExtractionResult(watermarkedBlock, compressedImageName, innerKey, sip, code, gridSize, RBWidth, Rxy, Bxy, gridPositionForEachBlock):
	isExtracted,key1,key2,key3 = extractRecursiveSip(watermarkedBlock, compressedImageName, innerKey, sip, code, gridSize, RBWidth, Rxy, Bxy, gridPositionForEachBlock)
